...
| **`runner`** | A machine connected via the runner daemon | `Runner` | Driving your own hardware remotely — see [Runner Nodes](./08_runner-nodes.md) |
...
Under **Settings → Runtime Providers** you can supply your own credentials so sandbox usage bills to your account, not ours.
...
Credentials are stored **in your browser's local storage** and sent directly from the browser to the provider when creating/stopping a sandbox. They are **never transmitted to or stored on the portal backend**. If you don't supply a key, the hosted tier is used instead.
...
- [Cloud Environments](./06_environments.md) — the three categories and parent/child environments
- [Syncing Changes](./07_syncing-changes.md) — Git sync vs Workspace Sync
- [Runner Nodes](./08_runner-nodes.md) — connecting your own machine
